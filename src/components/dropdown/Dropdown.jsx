...
class Dropdown extends Component {
	...
	selectItem = (item) => {
		...
		this.setState(
			...
			() => selectedItem?.value !== value && onChange && onChange(item, name)
		);
	};
	...
	listItems = () => {
		...
		if (tempList.length) {
			return tempList.map((item) => (
				<button
					type="button"
					className={`dd-list-item ${item.value === selectedItemValue ? 'selected' : ''} ${id}`}
					style={listItem}
					key={item.value}
					onClick={() => this.selectItem(item)}>
					{item.label} {item.value === selectedItemValue && <span style={styles.checkIcon}>{checkIcon}</span>}
				</button>
			));
		}
